...
function transmit() {
  ...
  destination=$(printf "%s" "$destination" | sed 's/ /\\ /g')
	...
	rsync -a --info=progress2 "$PWD" "${username}"@"${IPS[$((TARGET_NODE - 1))]}":~/AirPlonk/"$destination" \
...
function help() {
	 ...
	 printf "       PLEASE NOTE: If the cargo directory has the same name as another \n"
	 printf "                    directory (in ~/AirPlonk) on the target node, that directory\n"
	 printf "                    (on the target node) will be overwritten.\n"
   usage
	 printf "\n\e[1mBUGS\e[0m\n"
